    ...
    { var g0128 *ClaireClass = ToClass(self.Id())
      { var n int = g0128.Open
        { 
          ...
          v_or4 = Equal(MakeInteger(n).Id(),MakeInteger(ClEnv.Open).Id())
          if (v_or4 == CTRUE) {Result = CTRUE
          } else { 
            v_or4 = Equal(MakeInteger(n).Id(),MakeInteger(ClEnv.Final).Id())
            if (v_or4 == CTRUE) {Result = CTRUE
            } else { 
              v_or4 = Equal(MakeInteger(n).Id(),MakeInteger(ClEnv.Close).Id())
              if (v_or4 == CTRUE) {Result = CTRUE
              } else { 
                { 
                  var v_and8 *ClaireBoolean
                  
                  v_and8 = Equal(MakeInteger(n).Id(),MakeInteger(ClEnv.ABSTRACT).Id())
                  if (v_and8 == CFALSE) {v_or4 = CFALSE
                  } else { 
                    { var arg_2 *ClaireAny
                      { 
                        var c *ClaireClass
  ...
  if (C_class.Id() == y.Isa.Id()) { 
    ...
    { var g0144 *ClaireTuple = ToTuple(y)
      { 
        ...
        v_and3 = Equal(C_tuple.Id(),x.Isa.Id())
        if (v_and3 == CFALSE) {Result = EID{CFALSE.Id(),0}
        } else { 
          v_and3 = Equal(ANY(F_CALL(C_length,ARGS(x.ToEID()))),MakeInteger(g0144.Length()).Id())
          if (v_and3 == CFALSE) {Result = EID{CFALSE.Id(),0}
          } else { 
            var try_1 EID
            { var arg_2 *ClaireAny
...
func F_set_I_Param (x *ClaireParam) EID { 
  var Result EID
  { var y_out *ClaireSet = ToType(CEMPTY.Id()).EmptySet()
    { 
      var y *ClaireAny
      _ = y
      Result= EID{CFALSE.Id(),0}
...
func F__dot_dot_integer_type (x *ClaireType,y *ClaireType) EID { 
  ...
  { 
    ...
    v_and1 = F_unique_ask_type(x)
    if (v_and1 == CFALSE) {try_1 = EID{CFALSE.Id(),0}
    } else { 
      v_and1 = F_unique_ask_type(y)
      if (v_and1 == CFALSE) {try_1 = EID{CFALSE.Id(),0}
      } else { 
        var try_2 EID
        { var arg_3 *ClaireAny
...
func F_but_any (s *ClaireAny,x *ClaireAny) EID { 
  ...
  if (s.Isa.IsIn(C_list) == CTRUE) { 
    { var g0172 *ClaireList = ToList(s)
      { var y_out *ClaireList = ToType(CEMPTY.Id()).EmptyList()
        { 
          var y *ClaireAny
          _ = y
...
func F_glb_class (x *ClaireClass,y *ClaireType) *ClaireType { 
  ...
  if ((x.Open == ClEnv.ABSTRACT) && 
    ...
    { var arg_1 *ClaireList
      { 
        var v_list3 *ClaireSet
        var z *ClaireType
        var v_local3 *ClaireAny
        v_list3 = x.Subclass
        arg_1 = CreateList(ToType(CEMPTY.Id()),v_list3.Length())
        for CLcount := 0; CLcount < v_list3.Count; CLcount++{ 
          z = ToType(v_list3.At(CLcount))
          v_local3 = ANY(F_CALL(ToProperty(C_glb.Id()),ARGS(EID{z.Id(),0},EID{y.Id(),0})))
